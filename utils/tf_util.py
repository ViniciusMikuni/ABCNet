...
def pairwise_distanceR(point_cloud):
  ...
  pt = tf.expand_dims(pt,-1)
  is_zero = point_cloud[:,:,] 
  point_shift = 1000*tf.where(tf.equal(pt,0),tf.ones_like(pt),tf.fill(tf.shape(pt), tf.constant(0.0, dtype=pt.dtype))) #Dont consider zero padded particles to the calculation
